import glob
from os import listdir,path,walk, makedirs,walk
import random
import argparse, shutil

def argparser():
    parser = argparse.ArgumentParser(prog='data clean')
    parser.add_argument('--data_dir',   type = str, default = r'D:\data\Malaysia\val_clean_label_only_one_line', required=False,
                        help='Path to input data\n' )
    parser.add_argument('--mode', default = "i", type = str,
                        help='random with folder(f) or image file(i)\n')
    parser.add_argument('--train_rate', default=[1.0], type=list, help='the rate of train data')
    parser.add_argument('--copy_file', default=0, type=int, help='0:not copy, 1:copy file, 2:move file')
    parser.add_argument('--out_path', default=r'D:\data\all_TD\EVS_train', type=str, help='out path')
...
def random_img(file_path, train_rate, out_path, copy_file):
    ...
    for root, dirs, files in walk(file_path):
        for img_files in glob.glob(path.join(root, "*.jpg")):
            txt_file_list.append(img_files)
    ...
    with open(path.join(file_path, "test.txt"), 'w') as test_file:
        test_file.write('\n'.join(test_list))
    if copy_file:
        makedirs(path.join(out_path, r'train'))
        makedirs(path.join(out_path, r'test'))
        for file in train_list:
            filename = path.basename(file)
            shutil.copyfile(file, path.join(path.join(out_path, r'train'), filename))
            shutil.copyfile(file[:-4]+'.txt', path.join(path.join(out_path, r'train'), filename[:-4]+'.txt'))
        for file in test_list:
            filename = path.basename(file)
            shutil.copyfile(file, path.join(path.join(out_path, r'test'), filename))
            shutil.copyfile(file[:-4] + '.txt', path.join(path.join(out_path, r'test'), filename[:-4] + '.txt'))
...
def random_folder(file_path, train_rate, out_path, copy_file):
    ...
    if num_train_rate ==1:
        ...
        if copy_file == 1:
            ...
            for folder in train_folders:
                folder_name = path.basename(folder)
                shutil.move(folder, "{}/train/{}/".format(out_path, folder_name))
            for folder in test_folders:
                folder_name = path.basename(folder)
                shutil.move(folder, "{}/test/{}/".format(out_path, folder_name))
